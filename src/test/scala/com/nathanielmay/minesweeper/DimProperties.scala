...
object DimProperties extends Properties("A Dim") {

  property("must be at least 1x1 and less than sqrt(Int.MaxValue)") = forAll {
  ...
  property("less than 1x1 and more than sqrt(Int.MaxValue) are empty") = forAll {
    (h: Int, v: Int) =>
  ...
  property("at least 1x1 and less than sqrt(Int.MaxValue) are defined") = forAll(goodDim, goodDim) {
    (h: Int, v: Int) =>
      !(List(h, v, h*v).exists(_ < 1) || List(h, v).exists(_ > scala.math.sqrt(Int.MaxValue))) ==>
